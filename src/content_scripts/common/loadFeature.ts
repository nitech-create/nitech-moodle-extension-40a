...
const sortFeatures = function(features: Feature[]) {
  ...
  if(new Set(featureNames).size !== features.length) {
    throw Error(`Multiple features has the same unique name or features duplicated`);
  }
  ...
  const visit = function(feature: Feature, localVisited: Set<string>) {
    ...
    if(localVisited.has(feature.uniqueName)) {
      throw Error(`Circular dependency detected on resolving feature ${feature.uniqueName}`);
    }
    ...
    for(const depFeatureName of feature.dependencies ?? []) {
      ...
      if(!depFeature) {
        throw Error(`Feature ${depFeatureName} is not provided to feature loader`);
      }
...
const loadFeature = function(features: Feature[], contextUrl: URL, showLog = false) {
  ...
  for(const feature of sortedFeatures) {
    const depFeaturePromises = (feature.dependencies ?? []).map((name) => loaderPromiseMap.get(name));
    if(depFeaturePromises.some((v) => v === undefined)) {
      throw Error(`Failed to resolve feature ${feature.uniqueName}: dependency feature does not exist`);
    }
    ...
    loaderPromiseMap.set(feature.uniqueName, Promise.all(depFeaturePromises).then(() => {
      if(!testByStringOrRegExp(feature.hostnameFilter, contextHost)) {
        if(showLog) {
          console.log(`[FeatureLoader] Skipping ${feature.uniqueName}: hostname does not match`);
        }
        return;
      }
      if(!testByStringOrRegExp(feature.pathnameFilter, contextPath)) {
        if(showLog) {
          console.log(`[FeatureLoader] Skipping ${feature.uniqueName}: pathname does not match`);
        }
        return;
      }
      ...
      if(feature.propagateError === false) {
        ...
        return Promise.resolve(feature.loader()).catch((err: unknown) => {
          console.warn(`Uncaught error in feature loader ${feature.uniqueName}: `, err);
        });
      }

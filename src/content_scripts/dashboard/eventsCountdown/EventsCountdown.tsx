...
const Countdown = (props: ArrayItem<EventsCountdownProps["items"]>) => {
  ...
  const duration = Math.floor((props.expireDate.getTime() - currentTime) / 1000);
  setTimeout(() => setCurrentTime(Date.now()), msUntilNextUpdate(Math.abs(duration), currentTime));
  ...
  return createPortal((
    <span className={className}>
      {duration >= 0 ? `残り${createDurationText(duration)}` : `${createDurationText(-duration)}前`}
    </span>
  ), props.portalTarget)
};

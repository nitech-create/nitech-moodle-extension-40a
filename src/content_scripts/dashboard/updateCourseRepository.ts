import { Course, storeCourseByMerge } from '../../common/storage/course.ts';
import type { Feature } from '../common/types.ts';
import waitForPageLoad from './waitForPageLoad.ts';

const regularCourseRegExp = /^(.+)\s*(\d{4})(\d)(\d{4})\s*(前期|後期)\s*((?:日|月|火|水|木|金|土)曜)\s*(\d)-(\d\d?)限/;
const semesterMap = {
  '前期': '1/2' as const,
  '後期': '2/2' as const
};
...
const updateCourseRepository: Feature<void, void> = {
  ...
  loader: () => {
    ...
    if(!elMyOverview) {
      throw Error(`[${updateCourseRepository.uniqueName}] Failed to get "my overview" section`);
    }
